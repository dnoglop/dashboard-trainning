// src/views/TreinamentosView.tsx
...
const TreinamentosView: React.FC = () => {
  ...
  const kpiData = useMemo((): KPIData[] => {
    // ... (toda a sua lógica de KPI, que deve funcionar agora)
    ...
    const concluidos = treinamentos.filter(
      (t) => t.status === TrainingStatus.CONCLUIDO,
    );
    const totalConcluidos = concluidos.length;

    const comSatisfacao = concluidos.filter(
      (t) => typeof t.satisfaction_score === "number",
    );
    const mediaSatisfacao =
      comSatisfacao.length > 0
        ? (
            comSatisfacao.reduce((sum, t) => sum + t.satisfaction_score!, 0) /
            comSatisfacao.length
          ).toFixed(1) + "/5"
        : "N/A";

    const comParticipantes = treinamentos.filter(
      (t) =>
        typeof t.number_of_participants === "number" &&
        typeof t.max_participants === "number" &&
        t.max_participants! > 0,
    );
    const taxaOcupacaoMedia =
      comParticipantes.length > 0
        ? (
            (comParticipantes.reduce(
              (sum, t) => sum + t.number_of_participants! / t.max_participants!,
              0,
            ) /
              comParticipantes.length) *
            100
          ).toFixed(0) + "%"
        : "N/A";
    ...
    return [
      {
        ...
        value: `R$ ${totalInvestimento.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
  ...
  const trainingStatusDistribution = useMemo(() => {
    ...
    Object.values(TrainingStatus).forEach((status) => (counts[status] = 0));
  ...
  const treinamentoColumns = [
    {
      key: "training_name" as keyof Treinamento,
    ...
    {
      key: "status" as keyof Treinamento,
      header: "Status",
      render: (item: Treinamento) => <StatusBadge status={item.status} />,
    },
    { key: "category" as keyof Treinamento, header: "Categoria" },
    {
      key: "target_audience" as keyof Treinamento,
      header: "Público Alvo",
      render: (item: Treinamento) => (
        <span className="text-sm text-gray-600">{item.target_audience}</span>
      ),
    },
    {
      key: "participants" as keyof Treinamento,
      header: "Participantes",
      render: (item: Treinamento) =>
        typeof item.number_of_participants === "number" &&
        typeof item.max_participants === "number"
          ? `${item.number_of_participants}/${item.max_participants}`
          : item.max_participants
            ? `Ate ${item.max_participants}`
            : "N/A",
    },
    { key: "duration_hours" as keyof Treinamento, header: "Duração (H)" },
    {
      key: "cost" as keyof Treinamento,
      header: "Custo",
      render: (item: Treinamento) =>
        `R$ ${item.cost.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
    },
    {
      key: "satisfaction_score" as keyof Treinamento,
      header: "Satisfação",
      render: (item: Treinamento) =>
        item.satisfaction_score ? `${item.satisfaction_score}/5` : "N/A",
    },
    { key: "provider" as keyof Treinamento, header: "Provedor" },
    {
      key: "training_date" as keyof Treinamento,
      header: "Data",
      render: (item: Treinamento) => formatDate(item.training_date),
    },
  ];
    ...
    <div className="p-4 md:p-8 space-y-8">
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          onClick={() =>
            alert(
              'Funcionalidade "Adicionar Novo Treinamento" em desenvolvimento!',
            )
          }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1">
          <ChartWrapper title="Distribuição de Status dos Treinamentos">
            {trainingStatusDistribution.filter((d) => d.value > 0).length >
            0 ? (
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
                    data={trainingStatusDistribution.filter((d) => d.value > 0)}
                    cx="50%"
                    cy="50%"
                    labelLine={false}
                    outerRadius="80%"
                  ...
                  >
                    {trainingStatusDistribution
                      ...
                      .map((entry, index) => {
                        ...
                        if (entry.name === TrainingStatus.CONCLUIDO)
                          color = "#00C49F";
                        if (entry.name === TrainingStatus.EM_ANDAMENTO)
                          color = "#FFBB28";
                        if (entry.name === TrainingStatus.PLANEJADO)
                          color = "#0088FE";
                        if (entry.name === TrainingStatus.CANCELADO)
                          color = "#FF8042";
              ...
              <p className="text-gray-500 text-center py-10">
                Sem dados para o gráfico de status.
              </p>
            )}
          </ChartWrapper>
        </div>
        <div className="lg:col-span-2">
          <ChartWrapper title="Custo Total por Categoria de Treinamento">
            {costByCategory.length > 0 ? (
              ...
              <p className="text-gray-500 text-center py-10">
                Sem dados para o gráfico de custos.
              </p>
            )}
          </ChartWrapper>
        </div>
      ...
      <div>
        <h3 className="text-2xl font-semibold text-gray-700 mb-4">
          Lista Detalhada de Treinamentos
        </h3>
        <Table<Treinamento>
          data={treinamentos}
